...
static int __init servo_init(void){
	printk(KERN_INFO "SERVO1 : Starting ---\n");

	register_chrdev(240, "servo1_dev", &fops);

	gpio_request(A2,"A2");
	gpio_direction_output(A2,LOW);

	printk(KERN_INFO "SERVO1 : Starting Complete\n");
...
static void __exit servo_exit(void){
	...
	unregister_chrdev(240, "servo1_dev");

	printk(KERN_INFO "servo1 : Exit Done");
}
